...
// A second, wider timeout around the ENTIRE request — not just the call to
// Gemini. AI_TIMEOUT_MS above only bounds the Gemini fetch itself; if
// something stalls earlier (checking who's calling via Supabase Auth,
// reading the request body, anything before Gemini is even reached), that
// wouldn't be caught by AI_TIMEOUT_MS at all and the function could still
// hang. This one wraps the whole handler so every request is bounded no
// matter where it gets stuck.
const GLOBAL_TIMEOUT_MS = 32000
...
Deno.serve(async (req) => {
  ...
  // Whatever handleRequest() ends up doing, this makes sure a *response*
  // comes back within GLOBAL_TIMEOUT_MS no matter what — including cases
  // AI_TIMEOUT_MS can't catch, like a stall in the auth check before
  // Gemini is ever reached. (This can't cancel a stuck call already in
  // flight inside handleRequest — Deno doesn't support that — but it
  // guarantees the caller isn't left waiting indefinitely either way.)
  ...
  const globalTimeout = new Promise<Response>((resolve) => {
    globalTimeoutId = setTimeout(() => {
      resolve(
        json(
          {
            error: `This function took longer than ${Math.round(
              GLOBAL_TIMEOUT_MS / 1000
            )}s to respond at all — which means whatever's slow is happening BEFORE the request to Gemini even goes out (most likely: checking who's calling, via Supabase's own auth), not Gemini itself being slow. Check Edge Functions → ai-assistant → Logs in your Supabase dashboard right after trying again — the newest log entry will show exactly which step it's stuck on.`,
          },
          504
        )
      )
  ...
  try {
    return await Promise.race([handleRequest(req), globalTimeout])
  } finally {
