...
-- `voices` here is only ever id/name/duration/path metadata — never the
-- actual audio bytes. That's safe to hand back to any visitor with the
-- link, same as the title/content already are; a visitor still can't
-- actually listen to a memo from this data alone, since the storage
-- bucket itself stays private. Playing one requires a second step (the
-- get-shared-voice-url Edge Function) which independently re-checks the
-- token and the exact path before minting a short-lived signed URL.
...
returns table (
  ...
  tags jsonb, content text, share_permission text, voices jsonb
)
security definer
