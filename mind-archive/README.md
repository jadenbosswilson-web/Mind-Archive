...
- **`get-shared-voice-url-function.ts`** — required for voice memos to be
  playable on shared notes (see "Sharing notes"); without it, shared notes
  still show everything else, just not their audio.
- **`README.md`** — this file.
...
- **Voice memos ARE playable** on a shared note, view-only or editable —
  as long as you've deployed `get-shared-voice-url-function.ts` (see
  below). Without it, everything else on a shared note still works fine,
  the voice memo list just won't be able to load audio for visitors.
...
**Making voice memos playable on shared notes:** the `voice-memos` Storage
bucket stays private (only the owner can read a file directly), so a
visitor opening a share link — who may not even be logged in — can't
generate playable audio on their own. Deploy `get-shared-voice-url-function.ts`
as a Supabase Edge Function (same "paste and deploy from the dashboard"
process as `delete-account-function.ts` above — full instructions are
inside that file) and this is solved: each play tap asks that function for
a short-lived signed URL, which it only ever hands out after independently
re-checking that the token is valid, the note isn't locked, and the
requested file genuinely belongs to that note. Skip this step and shared
notes still work perfectly — the voice memo list will just show but its
play buttons will report "Could not load this recording" until you do.
