...
- **`vercel.json`** — a small config file so Vercel knows to serve
  `mind-archive-app.html` as your homepage, and to make shared note links
  (`/n/...`) work too (see Part 3).
...
2. Select **both** `mind-archive-app.html` and `vercel.json` from this folder
   ...
   `vercel.json` file tells Vercel "serve `mind-archive-app.html` for
   *every* URL on this site" — needed not just for the homepage, but for
   shared note links too (`/n/...`), since this is all one file with the
   page itself deciding what to show based on the URL. Without it, Vercel
   will ask you to pick a homepage, but shared note links would still 404.
...
- **A shared note link (or the homepage itself) 404s** — make sure the
  current `vercel.json` is actually in your repo/deploy at the same folder
  as `mind-archive-app.html`, and that you've re-deployed after replacing
  it (Vercel doesn't retroactively apply a config change to old
  deployments — push again, or **Deployments → ⋯ → Redeploy**).
