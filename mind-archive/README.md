...
"Share note" generates a genuine public, read-only link (like
`https://your-site.vercel.app/n/<a random id>`) that anyone can open — no
account or login required — to view that one note. A few things worth
knowing:

- **It's read-only.** Whoever opens the link can't edit or delete anything,
  and they can't see any of your other notes, folders, or account info.
- **Locked notes can't be shared.** If a note has a lock password on it,
  the Share button will tell you to unlock it first — sharing would defeat
...
- **Voice memos aren't included** in a shared note's public view, only the
  text content — same as the "paste content to me" workflow in Part 4,
  audio stays account-only for now.
- Under the hood: each shared note gets a random token stored in a new
  `share_token` column (`schema.sql`), and a database function only ever
  returns *one* note for the *exact* token it's given — there's no way for
  a visitor to browse or list every shared note on the site, only the
  specific one they were sent a link to.
...
- **A shared note link says "This link isn't available" even though you
  ...
  `share_token` column and the function that serves shared notes; if your
  database was set up before this update, that piece is missing).
- **A shared note link 404s instead of showing "This link isn't
  available"** — make sure the current `vercel.json` (with the `/n/:token`
  rule) is actually deployed — re-deploy after replacing it, same as any
  other file update.
