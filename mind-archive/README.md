...
- **`manifest.json`, `icon-192.png`, `icon-512.png`, `icon-180.png`, `sw.js`**
  ...
  to sit in the same folder as `mind-archive-app.html` when you deploy.
...
1. Push this folder's files to a GitHub repo (`mind-archive-app.html`,
   `vercel.json`, and `schema.sql` at minimum).
...
4. **Root Directory:** point this at whichever folder inside the repo
   actually contains `mind-archive-app.html` and `vercel.json` — they both
   need to be in that same folder.
5. Leave Build Command and Output Directory empty/default, then **Deploy**.

**The most common cause of a "404 NOT_FOUND"** on this method is
`vercel.json` not being in the repo at that Root Directory — without it,
Vercel has no `index.html` to fall back to and doesn't know
`mind-archive-app.html` should be the homepage. Double-check it's actually
committed and pushed, in the exact same folder, then trigger a redeploy
(push again, or **Deployments → ⋯ → Redeploy**).

To update the live site later with this method, just push a new commit with
the updated `mind-archive-app.html` — Vercel deploys it automatically.
...
deployed alongside `mind-archive-app.html` (`manifest.json`, the `icon-*.png`
...
actually get deployed alongside `mind-archive-app.html` — double check
...
account or login required. You choose the permission right in the Share
dialog:
...
- **Can view** — read-only. Whoever opens the link can't edit or delete
  anything, and they can't see any of your other notes, folders, or
  account info.
- **Can edit** — they get an editable title + note body and a **Save
  changes** button, right there on the public page, no account needed.
  Their changes save back to your note. This is "last save wins," not
  live/real-time co-editing — if two people have the edit link open at
  once, whoever clicks Save last is what sticks, similar to editing a
  shared file without watching each other type.

A few things worth knowing either way:

- **Locked notes can't be shared.** If a note has a lock password on it,
  the Share button will tell you to unlock it first — sharing (either
  permission) would defeat the point of locking it.
- **The link doesn't change every time you open the Share dialog** — it's
  ...
  invalidates it (anyone with the old link gets "this link isn't
  available" from then on, whether they had view or edit access).
- **Switching from "Can edit" back to "Can view"** takes effect
  immediately — anyone who still has the link can only view from then on,
  even without a new link.
...
- Under the hood: each shared note gets a random token stored in a new
  `share_token` column, plus a `share_permission` column (`schema.sql`).
  Two database functions handle everything: one only ever returns *one*
  note for the *exact* token it's given (no browsing/listing shared notes
  in bulk), and the other only ever updates *one* note, and only if its
  permission is currently set to "edit."
...
call a real AI model (Claude) for every response — there's no
canned/pattern-matched text left anywhere in the app. This needs one more
optional piece deployed, the same way "Delete account" did:

1. Get an API key from [console.anthropic.com](https://console.anthropic.com)
   (Anthropic, the company that makes Claude) — this is a separate account
   from Supabase, and Anthropic bills you directly for usage (new accounts
   get a small free credit, then it's pay-as-you-go; normal use of this app
   costs a small fraction of a cent per message).
...
**Already deployed `ai-assistant` before and just got a newer
`ai-assistant-function.ts`?** It now also handles auto-tag and the weekly
digest — go back to Supabase → Edge Functions → `ai-assistant` → replace
the code with the new file's contents → **Deploy function** again. Your
`ANTHROPIC_API_KEY` secret stays as-is, nothing else to redo. Also re-run
the current `schema.sql` once (it adds two small columns the digest needs
to save itself — safe to run again).
...
- **Privacy note:** using any of these sends the relevant text (your chat
  messages, or a note's content) to Anthropic's API to generate a reply —
  that's inherent to what "real AI" means here, the same as it would be for
  any AI feature in any app. If you'd rather a note never leave your
  account, don't use the AI buttons on it.
...
- **The homepage itself 404s** — make sure the current `vercel.json` is
  actually in your repo/deploy at the same folder as `mind-archive-app.html`,
  and that you've re-deployed after replacing it (Vercel doesn't
  retroactively apply a config change to old deployments — push again, or
...
- **"Can edit" link loads fine but Save changes doesn't stick** — usually
  ...
  `update_shared_note` function doesn't exist yet), or the note was
  switched back to "Can view" (or unshared) after the link was sent out.
...
- **AI chat or the per-note AI buttons say "AI isn't set up on this
  ...
  and add your `ANTHROPIC_API_KEY` secret; see "Turning on real AI" above.
- **AI chat/actions say "Unauthorized" or "AI request failed (401/403)"**
  — the `ANTHROPIC_API_KEY` secret is missing, mistyped, or the key was
  revoked in your Anthropic console; re-check it under Supabase → Edge
