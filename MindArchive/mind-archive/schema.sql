...
-- ============================================================
-- Note sharing — real public, read-only links ("anyone with the
-- link can view", no account needed to view).
--
...
-- to. Instead, get_shared_note() only ever returns a single row, and
-- only when called with the exact matching token — there's no way to
-- browse or enumerate shared notes in bulk.
-- ============================================================
...
language sql
as $$
  select title, date, mood, topic, activity, tags, content
  from notes
